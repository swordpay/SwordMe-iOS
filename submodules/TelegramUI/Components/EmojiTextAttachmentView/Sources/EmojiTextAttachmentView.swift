...
public final class InlineStickerItemLayer: MultiAnimationRenderTarget {
    ...
    private func updateTopicInfo(topicInfo: (Int64, EngineMessageHistoryThread.Info)) {
        if topicInfo.0 == 1 {
            ...
            if let image = generateTopicIcon(title: String(topicInfo.1.title.prefix(1)), backgroundColors: colors.0.map { UIColor.init(rgb: $0) }, strokeColors: colors.1.map { UIColor.init(rgb: $0) }, size: CGSize(width: 32.0, height: 32.0)) {
                self.contents = image.cgImage
            }
        }
    }
