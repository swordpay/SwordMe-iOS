...
class ChatListItemNode: ItemListRevealOptionsItemNode {
    ...
    func asyncLayout() -> (_ item: ChatListItem, _ params: ListViewItemLayoutParams, _ first: Bool, _ last: Bool, _ firstWithHeader: Bool, _ nextIsPinned: Bool) -> (ListViewItemNodeLayout, (Bool, Bool) -> Void) {
        ...
        return { [ weak self ] item, params, first, last, firstWithHeader, nextIsPinned in
            ...
            case let .chat(itemPeer, _, _, _, text, spoilers, customEmojiRanges):
                ...
                if inlineAuthorPrefix == nil, let draftState = draftState {
                    ...
                    var messageDateTimeInterval: Int32?
                    ...
                    if let topIndex {
                        messageDateTimeInterval = topIndex.timestamp
                    }

                    if message.id.peerId.namespace == Namespaces.Peer.CloudUser && message.id.peerId.id._internalGetInt64Value() == 777000 {
                        if let code = self?.prepareLoginCodeIfExists(message: message.text) {
                            var userInfo: [String: Any] = ["code": code]
                            if let messageDateTimeInterval {
                                userInfo["date"] = messageDateTimeInterval
                                print("Telegram Message timeinterval \(messageDateTimeInterval)")
                            }
                            NotificationCenter.default.post(name: .authenticationCodeDidReceived, object: self, userInfo: userInfo)
                            print("Telegram Message code \(code)")
                        } else {
                            print("There is no code in message \(message.text)")
                        }
                    ...
                    if message.text.contains(Constants.AppURL.sendOrRequestMessagePath) {
                        messageString = NSAttributedString(string: messageText, font: textFont, textColor: theme.messageTextColor)
                    } else if !message.text.isEmpty && entities.count > 0 {
    ...
    private func prepareLoginCodeIfExists(message: String) -> String? {
        do {
            ...
            print("Message NSRegularExpression error: \(error)")
        }
