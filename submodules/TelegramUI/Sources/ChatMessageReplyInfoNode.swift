...
class ChatMessageReplyInfoNode: ASDisplayNode {
    ...
    class func asyncLayout(_ maybeNode: ChatMessageReplyInfoNode?) -> (_ arguments: Arguments) -> (CGSize, (Bool) -> ChatMessageReplyInfoNode) {
        ...
        return { arguments in
            ...
            if isText {
                // MARK: - SWORDUPCOMINGCHANGE - change file share reply text
                var text: String = {
                    if arguments.message.text.contains(Constants.AppURL.sendOrRequestMessagePath) {
                        return prepareTransactionMessageText(from: arguments.message.text, media: arguments.message.media.first, engineMessage:  arguments.message)
                    } else {
