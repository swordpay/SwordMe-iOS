...
public enum ExportedInvitation: Codable, Equatable {
    ...
    public func withUpdatedLink() -> ExportedInvitation {
        let swordMeDomain = "t.me"
        switch self {
            case let .link(link, title, isPermanent, requestApproval, isRevoked, adminId, date, startDate, expireDate, usageLimit, count, requestedCount):
            return .link(link: link.replacingOccurrences(of: "t.me", with: swordMeDomain), title: title, isPermanent: isPermanent, requestApproval: requestApproval, isRevoked: isRevoked, adminId: adminId, date: date, startDate: startDate, expireDate: expireDate, usageLimit: usageLimit, count: count, requestedCount: requestedCount)
            case .publicJoinRequest:
                return .publicJoinRequest
        }
    }
