...
public class ChatListControllerImpl: TelegramBaseController, ChatListController {
    // MARK: - SwordChanges
    private var transactionsCancellable: AnyCancellable? = nil
    private var cancellables: Set<AnyCancellable> = []
    private var footerViewModel: ChannelsActionsBottomViewModel = .init()
    private var transactionDispossable: Disposable?
    private var qrResultPublisher: PassthroughSubject<String?, Never> = .init()
    ...
    private let headerContentView = ComponentView<ComponentFlow.Empty>()
    ...
    private var tfaAuthorizationDisposable: Disposable?
    private var chatDisposable: Disposable?

    private var statusController: ViewController?

    private var isGroupIdRoot: Bool {
        switch location {
    ...
    public init(context: AccountContext, location: ChatListControllerLocation, controlsHistoryPreload: Bool, hideNetworkActivityStatus: Bool = false, previewing: Bool = false, enableDebugActions: Bool) {
        ...
        let presentationDataCandidate = (context.sharedContext.currentPresentationData.with { $0 })
        let newRootController = presentationDataCandidate.theme.rootController.withUpdated(navigationBar: presentationDataCandidate.theme.rootController.navigationBar.withUpdated(separatorColor: UIColor(rgb: 0xF6F8FC)))
        
        self.presentationData = presentationDataCandidate.withUpdated(theme: presentationDataCandidate.theme.withUpdated(rootController: newRootController))
        ...
        DispatchQueue.main.asyncAfter(deadline: .now() + 3) {
            self.presentationData = self.presentationData.withUpdated(theme: self.presentationData.theme.withUpdated(rootController: self.presentationData.theme.rootController.withUpdated(statusBarStyle: .black)))
            
            let _ = self.context.sharedContext.currentPresentationData.swap(self.presentationData)
            ...
            case let .chatList(groupId):
                if groupId == .root {
                    ...
                    if useSpecialTabBarIcons() {
                        icon = UIImage(bundleImageName: "Chat List/Tabs/tabbar-channels-icon")
                    } else {
                        icon = UIImage(bundleImageName: "Chat List/Tabs/tabbar-channels-icon")
                    }
...
//                    }
                    ...
                    self.primaryContext?.rightButton = AnyComponentWithIdentity(id: "qrscanner", component: AnyComponent(NavigationButtonComponent(
                        content: .image(image: UIImage(imageName: "qr-scan-backgrounded-icon")!.withRenderingMode(.alwaysOriginal), sideSize: nil),
                        pressed: { [weak self] _ in
...
//                    strongSelf.chatListDisplayNode.effectiveContainerNode.currentItemNode.scrollToPosition(.top)
                    strongSelf.chatListDisplayNode.effectiveContainerNode.scrollToTop()
        ...
        |> deliverOnMainQueue).start(next: { [weak self] presentationData in
            if let strongSelf = self {
                ...
                let newRootController = presentationData.theme.rootController.withUpdated(navigationBar: presentationData.theme.rootController.navigationBar.withUpdated(separatorColor: UIColor(rgb: 0xF6F8FC)))

                strongSelf.presentationData = presentationData.withUpdated(theme: presentationData.theme.withUpdated(rootController: newRootController))
        ...
        if !previewing {
            ...
            if case .forum = location {
                ...
                placeholder = Constants.Localization.Common.users
                compactPlaceholder = Constants.Localization.Common.users
            }
            
            let newTheme = prepareSearchBarTheme()
            self.searchContentNode = NavigationBarSearchContentNode(theme: newTheme, placeholder: placeholder, compactPlaceholder: compactPlaceholder, activate: { [weak self] in
            ...
            self.activeDownloadsDisposable = stateSignal.start(next: { [weak self] state in
                ...
                if let animation = animation, let progressValue = progressValue {
                    let contentComponent = AnyComponent(ZStack<ComponentFlow.Empty>([
        ...
        if isGroupIdRoot {
            ...
            self.contactsDisposable = self.contactPeersViewPromise.get()
                .start(next: { [ weak self ]  (engineContactList: EngineContactList, _) in
                    ...
                    self.storeUserUsername(peerId: self.context.account.peerId)
                    AppData.userTelegramPeerId = self.context.account.peerId.id._internalGetInt64Value()
                    AppData.userTelegramExtraPeerId = self.context.account.peerId.toInt64()

                    self.peers = engineContactList.peers.filter { $0.id != self.context.account.peerId }
                    
                    let combinedData: [(EnginePeer, EnginePeer.Presence)] = engineContactList.peers
                        .filter { $0.smallProfileImage != nil && $0.id != self.context.account.peerId }
                        .compactMap {
                        guard let presence = engineContactList.presences[$0.id] else { return nil }
                        
                        return ($0, presence)
                    }

                    let sortedData = combinedData.sorted { lhs, rhs in
                        lhs.1.status > rhs.1.status
                    }

                    var recentUsersInfo: [ChannelsHeaderViewModel.Info] = []
                    
                    let timestamp = CFAbsoluteTimeGetCurrent() + NSTimeIntervalSince1970

                    var onlineUser = sortedData.prefix(while: {
                        if case let .present(until) = $0.1.status, until > Int32(timestamp) {
                            return true
                        } else {
                            return false
                        }
                    ...
                    let onlineUsersInfo: [ChannelsHeaderViewModel.Info] = onlineUser.map {
                        .init(enginePeer: $0.0,
                    ...
                    recentUsersInfo.append(contentsOf: onlineUsersInfo)
                    
                    let needExtraUsers = onlineUser.count < 10
                    
                    if needExtraUsers {
                        let extraUsers = sortedData.dropFirst(onlineUser.count).prefix(10 - onlineUser.count)

                        let recentylActiveUsers: [ChannelsHeaderViewModel.Info] = extraUsers.map {
                            .init(enginePeer: $0.0,
                                  account: context.account,
                                  context: context,
                                  theme: self.presentationData.theme,
                                  synchronousLoads: true,
                                  peerPresenceState: .recently)
                        }
                        
                        recentUsersInfo.append(contentsOf: recentylActiveUsers)
                        
                        onlineUser += extraUsers
                    }
                    })
        }
        ...
        bindToSelectedRecentItem()
        bindToPayOrRequestDidComplete()
        bindToAuthorizationCheckLoading()
    }
    ...
    deinit {
        ...
        self.contactsDisposable?.dispose()
        self.tfaAuthorizationDisposable?.dispose()
    }
    
    private func storeUserUsername(peerId: PeerId) {
        guard AppData.telegramUserUsername == nil else { return }
        ...
        transactionDispossable = context.account.postbox.transaction({ [ weak self ] transaction in
            ...
            if let peer = transaction.getPeer(peerId) {
                ...
                AppData.telegramUserUsername = userName.trimmingCharacters(in: .whitespacesAndNewlines).isEmpty ? (AppData.userPhoneNumber  ?? "") : userName
            } else {
                storeUserUsername(peerId: peerId)
            }
        })
        .start()
    }
    
    private func prepareSearchBarTheme() -> PresentationTheme {
        let newRootController = presentationData.theme.rootController.withUpdated(navigationSearchBar: presentationData.theme.rootController.navigationSearchBar.withUpdated(inputFillColor: UIColor(rgb: 0xF6F8FC)))
        return presentationData.theme.withUpdated(rootController: newRootController)
    }
    ...
    private func bindToAuthorizationCheckLoading() {
...
//            }
        ...
        let newTheme = prepareSearchBarTheme()
        self.presentationData = self.presentationData.withUpdated(theme: newTheme)
        self.searchContentNode?.updateThemeAndPlaceholder(theme: newTheme, placeholder: placeholder, compactPlaceholder: compactPlaceholder)
        ...
        self.statusBar.updateStatusBarStyle(self.presentationData.theme.rootController.statusBarStyle.style, animated: false)
...
//        AuthorizationManager.global.performSwordAuthorizationIfNeeded()
    }
...
//                })
    ...
    private func updateHeaderContent(layout: ContainerViewLayout, transition: ContainedViewLayoutTransition) {
        ...
        if let primaryContext = self.primaryContext {
            ...
            primaryContent = ChatListHeaderComponent.Content(
                ...
                titleComponent: primaryContext.chatTitleComponent.flatMap { AnyComponent<ComponentFlow.Empty>($0) },
        ...
        if let secondaryContext = self.secondaryContext {
            secondaryContent = ChatListHeaderComponent.Content(
                ...
                titleComponent: secondaryContext.chatTitleComponent.flatMap { AnyComponent<ComponentFlow.Empty>($0) },
    ...
    private func reloadFilters(firstUpdate: (() -> Void)? = nil) {
        ...
        |> deliverOnMainQueue).start(next: { [weak self] countAndFilterItems, peerView, limits in
            ...
            AppData.hasChatFolders = resolvedItems.count > 1
    ...
    private func activateSearch(filter: ChatListSearchFilter = .chats, query: String? = nil, skipScrolling: Bool = false) {
        ...
        if self.displayNavigationBar {
            ...
            |> deliverOnMainQueue).start(next: { [weak self] _, chatListView in
                ...
                strongSelf.footerView.isHidden = true
    ...
    private var qrResultPublisherCancellable: AnyCancellable?
    
    func goToQRScannerScreen() {
        let controller = CommonDestination.qrCodePager(scanResultPublisher: qrResultPublisher).viewController
        
        qrResultPublisherCancellable = qrResultPublisher
            .sink { [ weak self ] result in
                ...
                self.dismissNotFromRoot(animated: true) {
                    ...
                    NotificationCenter.default.post(name: .usernameDidSelected, object: self, userInfo: userInfo)
                    self.qrResultPublisherCancellable = nil
                }
            }
    ...
    private func setupFooterView() {
        ...
        bottomView.setup(with: footerViewModel)
        ...
        let bottom = UIApplication.shared.topMostViewController()?.view.safeAreaInsets.bottom ?? 0
        let bottomOffset: CGFloat = ScreenSize.currentScrennSize == .small || ScreenSize.currentScrennSize == .extraSmall ? 45 : tabBarHeight + bottom
        bottomView.addBorderConstraints(constraintSides: [.horizontal, .bottom], by: .init(top: 0, left: 0, bottom: -bottomOffset, right: 0))
        
        footerViewModel.payOrRequestButtonPublisher
            .sink { [ weak self ] in
                self?.openCompose()
            }.store(in: &cancellables)
        ...
        if stateAndFilterId.state.editing {
            ...
            if case .chatList(.root) = self.location {
                ...
                if isReorderingTabs {
                    ...
                    self.rightButton = AnyComponentWithIdentity(id: "qrscanner", component: AnyComponent(NavigationButtonComponent(
                        content: .image(image: UIImage(imageName: "qr-scan-backgrounded-icon")!.withRenderingMode(.alwaysOriginal), sideSize: nil),
                        pressed: { [weak self] _ in
                            self?.parentController?.goToQRScannerScreen()
                        }
                    )))
                }
...
extension ChatListControllerImpl {
    func bindToPayOrRequestDidComplete() {
        NotificationCenter.default.publisher(for: .payOrRequestDidComplete)
            .sink { [ weak self ] notification in
                guard let self,
                      let userInfo = notification.userInfo,
                      let id = userInfo["peerId"] as? Int64,
                      let extraId = userInfo["extraPeerId"] as? Int64 else { return }
                let peerId = PeerId(id)
                let extraPeerId = PeerId(extraId)
                
                let _ = combineLatest(
                    queue: Queue.mainQueue(),
                    self.context.engine.data.get(TelegramEngine.EngineData.Item.Peer.Peer(id: peerId)),
                    self.context.engine.data.get(TelegramEngine.EngineData.Item.Peer.Peer(id: extraPeerId))
                ).start(next: { peer, extraPeer in
                    let correctPeer = peer ?? extraPeer
                    guard let correctPeer else {
                        return
                    }

                        self.chatListDisplayNode.mainContainerNode.peerSelected?(correctPeer, nil, false , false, nil)
                    })
            }
            .store(in: &cancellables)
    }
    
    func bindToSelectedRecentItem() {
        NotificationCenter.default.publisher(for: .recentItemDidSelected)
            .sink { [ weak self ] notification in
                guard let self, let userInfo = notification.userInfo,
                let channelItem = userInfo["channelItem"] as? ChannelsHeaderViewModel.Info else { return }
                
                self.chatListDisplayNode.mainContainerNode.peerSelected?(channelItem.enginePeer, nil, false , false, nil)
            }
            .store(in: &cancellables)
    }
    
    private func bindToAppStateNotificaitons() {
        ...
        NotificationCenter.default.publisher(for: UIApplication.didBecomeActiveNotification)
            .sink { [ weak self ] _ in
                if AuthorizationManager.global.isLoading.value {
                    self?.showLoading()
                }
            }
            .store(in: &cancellables)
