...
// MARK: - Updated Phone screen page

private var supportEmail = "support@swordpay.com"
private enum InnerState: Equatable {
    case state(UnauthorizedAccountStateContents)
    case authorized
}

public final class AuthorizationSequenceController: NavigationController, MFMailComposeViewControllerDelegate, ASAuthorizationControllerDelegate, ASAuthorizationControllerPresentationContextProviding, LogoutManagable {
    ...
    private func swordPhoneNumberScreen() -> PhoneNumberViewController {
        let controller = ViewControllerProvider.Authentication.phoneNumber(verificationReason: .registration, otherAccountPhoneNumbers: self.otherAccountPhoneNumbers, loginWithNumber: nil)
        
            controller.viewModel.loginWithNumber = { [weak self, weak controller] number, syncContacts in
                
                AppData.accessToken = nil
                
                guard let self else {
                    return
                }
                controller?.viewModel.isLoading.send(true)
                        ...
                        strongSelf.actionDisposable.set((sendAuthorizationCode(accountManager: strongSelf.sharedContext.accountManager, account: strongSelf.account, phoneNumber: number, apiId: strongSelf.apiId, apiHash: strongSelf.apiHash, pushNotificationConfiguration: authorizationPushConfiguration, firebaseSecretStream: strongSelf.sharedContext.firebaseSecretStream, syncContacts: syncContacts, forcedPasswordSetupNotice: { value in
                            ...
                            if let strongSelf = self {
                                ...
                                case let .sentCode(account):
                                    controller?.viewModel.isLoading.send(false)
                                ...
                                switch error {
                                    ...
                                    case .invalidPhoneNumber:
                                        ...
                                        actions.append(TextAlertAction(type: .defaultAction, title: strongSelf.presentationData.strings.Login_PhoneNumberHelp, action: { [weak controller] in
                                            ...
                                            strongSelf.presentEmailComposeController(address: supportEmail, subject: strongSelf.presentationData.strings.Login_InvalidPhoneEmailSubject(formattedNumber).string, body: strongSelf.presentationData.strings.Login_InvalidPhoneEmailBody(formattedNumber, appVersion, systemVersion, locale, mnc).string, from: controller)
                                        }))
    ...
    private func swordCodeEntryController(number: String, phoneCodeHash: String, type: SentAuthorizationCodeType, nextType: AuthorizationCodeNextType?, termsOfService: (UnauthorizedAccountTermsOfService, Bool)?) -> VerifyPhoneNumberViewController {
        let controller = ViewControllerProvider.Authentication.verifyPhoneNumber(with: number,
                                                                                 ...
                                                                                 back: nil)
        controller.presentationData = presentationData
        
        controller.viewModel.loginInfo = { [ weak self, weak controller ] in
            guard let self, let controller else { return }
            
            controller.present(standardTextAlertController(theme: AlertControllerTheme(presentationData: self.presentationData), title: nil, text: "Starting February 18, 2023, updated Telegram authorization regulations are in place. Users of alternative clients can now obtain the SMS code exclusively through the official Telegram client. Kindly install the Telegram app and log into your account to receive the SMS code.", actions: [TextAlertAction(type: .defaultAction, title: self.presentationData.strings.Common_OK, action: {})]), in: .window(.root))
        }
        
        controller.viewModel.back = { [ weak self ] in
            guard let self else { return }

            let countryCode = AuthorizationSequenceController.defaultCountryCode()
            
            let _ = TelegramEngineUnauthorized(account: self.account).auth.setState(state: UnauthorizedAccountState(isTestingEnvironment: self.account.testingEnvironment, masterDatacenterId: self.account.masterDatacenterId, contents: .phoneEntry(countryCode: countryCode, number: ""))).start()
        }

        controller.viewModel.requestNextOption = { [weak self, weak controller] in
            if let strongSelf = self {
                if nextType == nil {
                    if MFMailComposeViewController.canSendMail(), let controller = controller {
                        ...
                        var emailBody = ""
                        emailBody.append(strongSelf.presentationData.strings.Login_EmailCodeBody(formattedNumber).string.replacingOccurrences(of: "Telegram", with: "Sword"))
                        emailBody.append("\n\n")
                        ...
                        let carrier = CTCarrier()
                        let mnc = carrier.mobileNetworkCode ?? "none"
                        emailBody.append("Sword Pay: \(appVersion)\n")
                        emailBody.append("OS: \(systemVersion)\n")
                        emailBody.append("Locale: \(locale)\n")
                        emailBody.append("MNC: \(mnc)")
                        
                        strongSelf.presentEmailComposeController(address: supportEmail, subject: strongSelf.presentationData.strings.Login_EmailCodeSubject(formattedNumber).string, body: emailBody, from: controller)
                    } else {
        ...
        controller.viewModel.loginWithCode = { [weak self, weak controller] code in
            if let strongSelf = self {
                controller?.viewModel.isLoading.send(true)
                ...
                if case let .email(_, _, _, _, _, setup) = type, setup, case let .emailVerification(emailCode) = authorizationCode {
                        ...
                        Queue.mainQueue().async {
                            if let strongSelf = self, let controller = controller {
                                ...
                                if case .invalidCode = error {
                                    controller.showWrongCodeAlert(text: strongSelf.presentationData.strings.Login_WrongCodeError)
                                } else {
                                    var resetCode = false
                                    let text: String
                                    switch error {
                    ...
                    |> deliverOnMainQueue).start(next: { result in
                        ...
                        controller?.shouldResetState = false
                        controller?.viewModel.isLoading.send(false)
                        switch result {
...
//                }
            ...
            if let strongSelf = self {
                if nextType == nil {
                    if MFMailComposeViewController.canSendMail(), let controller = controller {
                        ...
                        let carrier = CTCarrier()
                        let mnc = carrier.mobileNetworkCode ?? "none"
                        emailBody.append("SwordPay: \(appVersion)\n")
                        emailBody.append("OS: \(systemVersion)\n")
                        emailBody.append("Locale: \(locale)\n")
                        emailBody.append("MNC: \(mnc)")
                        
                        strongSelf.presentEmailComposeController(address: "support@swordpay.com", subject: strongSelf.presentationData.strings.Login_EmailCodeSubject(formattedNumber).string, body: emailBody, from: controller)
                    } else {
                        controller?.present(standardTextAlertController(theme: AlertControllerTheme(presentationData: strongSelf.presentationData), title: nil, text: strongSelf.presentationData.strings.Login_EmailNotConfiguredError, actions: [TextAlertAction(type: .defaultAction, title: strongSelf.presentationData.strings.Common_OK, action: {})]), in: .window(.root))
                    }
                } else {
    ...
    private func swordPasswordEntryController(hint: String, suggestReset: Bool, syncContacts: Bool) -> TFAPasswordViewController {
        let controller = ViewControllerProvider.Authentication.tfaPassword(passwordHint: hint, state: suggestReset ? .resetAccount : .forgotPassword)
        controller.presentationData = presentationData
        controller.viewModel.back = { [ weak self ] in
            guard let self else { return }
        ...
        controller.viewModel.loginWithPassword = { [weak self, weak controller] password in
            if let strongSelf = self {
                controller?.viewModel.isLoading.send(true)
        ...
        controller.viewModel.forgot = { [weak self, weak controller] in
            if let strongSelf = self, let strongController = controller {
                strongController.viewModel.isLoading.send(true)
                ...
                |> deliverOnMainQueue).start(next: { pattern in
                    if let strongSelf = self, let strongController = controller {
                        strongController.viewModel.isLoading.send(false)
...
//                    strongController.didForgotWithNoRecovery = true
        ...
        controller.viewModel.reset = { [weak self, weak controller] in
            if let strongSelf = self, let strongController = controller {
    ...
    private func swordPasswordRecoveryController(emailPattern: String, syncContacts: Bool) -> EmailRecoveryViewController {
        let controller = ViewControllerProvider.Authentication.emailRecovery(email: emailPattern)
        
        controller.viewModel.continueWithCode = { [ weak self, weak controller ] code in
            
            guard let self,
                  let controller else { return }

            controller.viewModel.isLoading.send(true)
            let engine: SomeTelegramEngine = .unauthorized(TelegramEngineUnauthorized(account: self.account))

            controller.actionDisposable.set((engine.auth.checkPasswordRecoveryCode(code: code)
            |> deliverOnMainQueue).start(error: { error in
                controller.viewModel.isLoading.send(false)
                ...
                switch error {
                case .limitExceeded:
                    text = self.presentationData.strings.LoginPassword_FloodError
                case .invalidCode:
                    text = self.presentationData.strings.Login_InvalidCodeError
                case .expired:
                ...
                controller.viewModel.isLoading.send(false)
                self.showResetPassword(parentController: controller, code: code, syncContacts: syncContacts)
            }))
        }

        controller.viewModel.cantAccessEmail = { [ weak self, weak controller ] in
            guard let self,
                  let controller else { return }
            
            controller.present(textAlertController(sharedContext: self.sharedContext, title: nil, text: self.presentationData.strings.TwoStepAuth_RecoveryFailed, actions: [TextAlertAction(type: .genericAction, title: self.presentationData.strings.Common_OK, action: {
                
                let _ = (TelegramEngineUnauthorized(account: self.account).auth.state()
                |> take(1)
                |> deliverOnMainQueue).start(next: { state in
                    if case let .unauthorized(state) = state, case let .passwordRecovery(hint, number, code, _, syncContacts) = state.contents {
                        let _ = TelegramEngineUnauthorized(account: self.account).auth.setState(state: UnauthorizedAccountState(isTestingEnvironment: self.account.testingEnvironment, masterDatacenterId: self.account.masterDatacenterId, contents: .passwordEntry(hint: hint, number: number, code: code, suggestReset: true, syncContacts: syncContacts))).start()
                    }
                })
            })]), in: .window(.root))
        }
        
        controller.viewModel.resendCode = { [ weak self, weak controller ] in
            guard let self,
                  let controller else { return }
            ...
            let engine: SomeTelegramEngine = .unauthorized(TelegramEngineUnauthorized(account: self.account))
            let _ = (engine.auth.requestTwoStepVerificationPasswordRecoveryCode()
            |> deliverOnMainQueue).start(error: { error in
                controller.viewModel.isLoading.send(false)
                ...
                switch error {
                case .limitExceeded:
                    text = self.presentationData.strings.TwoStepAuth_FloodError
                case .generic:
                    text = self.presentationData.strings.Login_UnknownError
                }
                
                controller.present(textAlertController(sharedContext: self.sharedContext, title: nil, text: text, actions: [TextAlertAction(type: .defaultAction, title: self.presentationData.strings.Common_OK, action: {})]), in: .window(.root))
            }, completed: {
                controller.viewModel.isLoading.send(false)
            })
        }
    ...
    private func showResetPassword(parentController: EmailRecoveryViewController, code: String, syncContacts: Bool) {
        
        let resetPasswordControlelr = ViewControllerProvider.Authentication.resetPassword
        
        resetPasswordControlelr.viewModel.skip = { [ weak self, weak resetPasswordControlelr ] in
            guard let self,
                  let resetPasswordControlelr else { return }
            
            resetPasswordControlelr.viewModel.isLoading.send(true)
            
            resetPasswordControlelr.present(standardTextAlertController(theme: AlertControllerTheme(presentationData: self.presentationData), title: self.presentationData.strings.TwoFactorSetup_PasswordRecovery_SkipAlertTitle, text: self.presentationData.strings.TwoFactorSetup_PasswordRecovery_SkipAlertText, actions: [
                TextAlertAction(type: .destructiveAction, title: self.presentationData.strings.TwoFactorSetup_PasswordRecovery_SkipAlertAction, action: {
                    self.performRecovery(parentController: resetPasswordControlelr, code: code, syncContacts: syncContacts)
                }),
                TextAlertAction(type: .defaultAction, title: self.presentationData.strings.Common_Cancel, action: {
                    resetPasswordControlelr.viewModel.isLoading.send(false)
                })
            ]), in: .window(.root))

        }
        
        resetPasswordControlelr.viewModel.recoverPassword = { [ weak self, weak resetPasswordControlelr ] (password, hint) in
            guard let self,
                  let resetPasswordControlelr else { return }

            performRecovery(parentController: resetPasswordControlelr, code: code, syncContacts: syncContacts, password: password, hint: hint)
        }

        guard let navigationController = parentController.navigationController as? NavigationController else {
            return
        }

        navigationController.replaceController(parentController, with: resetPasswordControlelr, animated: true)
    }

    private func performRecovery(parentController: ResetPasswordViewController, code: String, syncContacts: Bool, password: String? = nil, hint: String? = nil) {
        let engine: TelegramEngineUnauthorized = .init(account: account)
        let mappedMode: TwoFactorDataInputMode.Recovery.Mode = .notAuthorized(syncContacts: syncContacts)
        let recovery: TwoFactorDataInputMode.Recovery = .init(code: code, mode: mappedMode)
        
        let updatedPassword: UpdatedTwoStepVerificationPassword = {
            guard let password else {
                return .none
            }
            
            return .password(password: password, hint: hint ?? "", email: nil)
        }()
        
        parentController.viewModel.isLoading.send(true)

        let _ = (engine.auth.performPasswordRecovery(code: recovery.code, updatedPassword: updatedPassword)
        |> deliverOnMainQueue).start(next: { [ weak self, weak parentController ] recoveredAccountData in
            guard let strongSelf = self else { return }
            
            parentController?.viewModel.isLoading.send(false)

            strongSelf.handlePaswordRecovery(recoveredAccountData: recoveredAccountData, engine: engine, syncContacts: syncContacts)
        }, error: { [ weak self, weak parentController ] error in
            ...
            parentController?.present(textAlertController(sharedContext: self.sharedContext, title: nil, text: text, actions: [TextAlertAction(type: .defaultAction, title: self.presentationData.strings.Common_OK, action: {})]), in: .window(.root))
        }, completed: { [weak parentController] in
            parentController?.viewModel.isLoading.send(false)
        })
    }
    
    private func handlePaswordRecovery(recoveredAccountData: RecoveredAccountData, engine: TelegramEngineUnauthorized, syncContacts: Bool) {
        let _ = loginWithRecoveredAccountData(accountManager: self.sharedContext.accountManager,
                                             account: engine.account,
                                             recoveredAccountData: recoveredAccountData,
                                              syncContacts: syncContacts).start()
    }
                ...
                case .empty:
                    if let _ = self.viewControllers.last as? AuthorizationSequenceSplashController {
                        ...
                        if self.otherAccountPhoneNumbers.1.isEmpty {
                            ...
                            controllers.append(swordPhoneNumberScreen())
...
//                    controllers.append(self.phoneEntryController(countryCode: countryCode, number: number, splashController: previousSplashController))
                ...
                case let .confirmationCodeEntry(number, type, phoneCodeHash, _, nextType, _):
                    var controllers: [ViewController] = []
                    if !self.otherAccountPhoneNumbers.1.isEmpty {
                        controllers.append(self.splashController())
                    }
...
//                    controllers.append(self.phoneEntryController(countryCode: AuthorizationSequenceController.defaultCountryCode(), number: "", splashController: nil))
                    if case let .emailSetupRequired(appleSignInAllowed) = type {
                        ...
                        controllers.append(self.swordCodeEntryController(number: number, phoneCodeHash: phoneCodeHash, type: type, nextType: nextType, termsOfService: nil))
//                        controllers.append(self.codeEntryController(number: number, phoneCodeHash: phoneCodeHash, email: self.currentEmail, type: type, nextType: nextType, timeout: timeout, termsOfService: nil))
                ...
                case let .passwordEntry(hint, _, _, suggestReset, syncContacts):
                    ...
                    controllers.append(self.swordPasswordEntryController(hint: hint, suggestReset: suggestReset, syncContacts: syncContacts))
//                    controllers.append(self.passwordEntryController(hint: hint, suggestReset: suggestReset, syncContacts: syncContacts))
                ...
                case let .passwordRecovery(_, _, _, emailPattern, syncContacts):
                    ...
                    controllers.append(self.swordPasswordRecoveryController(emailPattern: emailPattern, syncContacts: syncContacts))
//                    controllers.append(self.passwordRecoveryController(emailPattern: emailPattern, syncContacts: syncContacts))
                ...
                case let .signUp(_, _, firstName, lastName, _, _):
                    ...
                    controllers.append(self.signUpController(firstName: firstName, lastName: lastName, termsOfService: nil, displayCancel: displayCancel))
                    self.setViewControllers(controllers, animated: !self.viewControllers.isEmpty)
            }
        }
    }
    ...
    public static func defaultCountryCode() -> Int32 {
        ...
        // MARK: - SwordChange this should be handled otherwise, mobileCountryCode is depricated in ios 16

//        let networkInfo = CTTelephonyNetworkInfo()
//        if let carrier = networkInfo.serviceSubscriberCellularProviders?.first?.value {
//            countryId = carrier.mobileCountryCode
//        }
