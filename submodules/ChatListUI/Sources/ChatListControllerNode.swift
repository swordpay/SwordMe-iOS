...
private final class ChatListContainerItemNode: ASDisplayNode {
    private final class TopPanelItem {
        let view = ComponentView<ComponentFlow.Empty>()
        var size: CGSize?
    ...
    public var listNodeIntialOffset: CGFloat?
    public var listTopInset: CGFloat?

    public var hasChatFolders: Bool? = nil
    public var isTabsVisible: Bool
    ...
    init(context: AccountContext, location: ChatListControllerLocation, filter: ChatListFilter?, chatListMode: ChatListNodeMode, previewing: Bool, isInlineMode: Bool, controlsHistoryPreload: Bool, presentationData: PresentationData, animationCache: AnimationCache, animationRenderer: MultiAnimationRenderer, isTabsVisible: Bool = false, becameEmpty: @escaping (ChatListFilter?) -> Void, emptyAction: @escaping (ChatListFilter?) -> Void, secondaryEmptyAction: @escaping () -> Void) {
        ...
        self.isTabsVisible = isGroupInRoot ? isTabsVisible : false
    ...
    func updateLayout(size: CGSize, insets: UIEdgeInsets, visualNavigationHeight: CGFloat, originalNavigationHeight: CGFloat, inlineNavigationLocation: ChatListControllerLocation?, inlineNavigationTransitionFraction: CGFloat, transition: ContainedViewLayoutTransition) {
        ...
        let bottomInset = ComponentSizeProvider.sendOrReqeustButtonHeight.size + 25
        let (duration, curve) = listViewAnimationDurationAndCurve(transition: transition)
        listInsets.bottom += bottomInset
        let updateSizeAndInsets = ListViewUpdateSizeAndInsets(size: size, insets: listInsets, duration: duration, curve: curve)

        transition.updateFrame(node: self.listNode, frame: CGRect(origin: .init(x: 0, y: 0),
                                                                  size: .init(width: size.width,
                                                                              height: size.height)))
        self.listNode.updateLayout(transition: transition, updateSizeAndInsets: updateSizeAndInsets,
                                   visibleTopInset: visualNavigationHeight + additionalTopInset,
                                   originalTopInset: originalNavigationHeight + additionalTopInset,
                                   inlineNavigationLocation: inlineNavigationLocation,
                                   inlineNavigationTransitionFraction: inlineNavigationTransitionFraction)
...
public final class ChatListContainerNode: ASDisplayNode, UIGestureRecognizerDelegate {
    ...
    var cachedRecents: [ChannelsHeaderViewModel.Info] = []
    ...
    private let isForForwarding: Bool
    public init(context: AccountContext, location: ChatListControllerLocation, chatListMode: ChatListNodeMode = .chatList(appendContacts: true), previewing: Bool, controlsHistoryPreload: Bool, isInlineMode: Bool, presentationData: PresentationData, animationCache: AnimationCache, animationRenderer: MultiAnimationRenderer, isForForwarding: Bool = false, filterBecameEmpty: @escaping (ChatListFilter?) -> Void, filterEmptyAction: @escaping (ChatListFilter?) -> Void, secondaryEmptyAction: @escaping () -> Void) {
        self.context = context
        ...
        self.selectedId = .all
        self.isForForwarding = isForForwarding
    ...
    public func scrollToTop() {
        if let itemNode = self.itemNodes[self.selectedId] {
            itemNode.listNode.adjustScrollOffsetForNavigation(isNavigationHidden: false)
...
final class ChatListControllerNode: ASDisplayNode, UIGestureRecognizerDelegate {
    ...
    func containerLayoutUpdated(_ layout: ContainerViewLayout, navigationBarHeight: CGFloat, visualNavigationHeight: CGFloat, cleanNavigationBarHeight: CGFloat, transition: ContainedViewLayoutTransition) {
        ...
        if let toolbar = self.toolbar {
            ...
            insets.bottom += (ComponentSizeProvider.sendOrReqeustButtonHeight.size + 24)
        ...
        // MARK: - SwordChanges
        transition.updateFrame(node: self.mainContainerNode, frame: CGRect(origin: .init(x: 0, y: 0), size: layout.size))
