...
public class SearchBarPlaceholderNode: ASDisplayNode {
    ...
    public init(fieldStyle: SearchBarStyle = .legacy) {
        ...
        self.backgroundNode.cornerRadius = 18
            ...
            return (height, { [weak self] in
                if let strongSelf = self {
                    ...
                    if let iconImage = strongSelf.iconNode.image {
                        ...
                        transition.updateFrame(node: strongSelf.iconNode, frame: CGRect(origin: CGPoint(x: leftSpacing, y: floorToScreenPixels((height - iconSize.height) / 2.0)), size: iconSize))
                    }
                    var textOffset: CGFloat = 0.0
                    if constrainedSize.height >= 36.0 {
                        textOffset += 2.5
                    }
                    let labelFrame = CGRect(origin: CGPoint(x: leftSpacing + iconSize.width + spacing, y: floorToScreenPixels((height - labelLayoutResult.size.height) / 2.0) + textOffset), size: labelLayoutResult.size)
