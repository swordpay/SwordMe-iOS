...
public final class NavigationButtonComponent: Component {
    ...
    public enum Content: Equatable {
        ...
        case image(image: UIImage, sideSize: CGFloat?)
    }
    ...
    public final class View: HighlightTrackingButton {
        ...
        private var iconImageSideSize: CGFloat?
        ...
        func update(component: NavigationButtonComponent, availableSize: CGSize, state: EmptyComponentState, environment: Environment<NavigationButtonComponentEnvironment>, transition: Transition) -> CGSize {
            ...
            case let .proxy(status):
                proxyStatus = status
            case let .image(image, sideSize):
                iconImage = image
                iconImageSideSize = sideSize
            }
            ...
            if let iconImage {
                ...
                iconView.tintColor = UIColor(rgb: 0x093489)
                let iconSideSize: CGFloat = iconImageSideSize ?? 35
                size.width = iconSideSize
                
                iconView.frame = CGRect(origin: CGPoint(x: iconOffset,
                                                        y: floor((availableSize.height - iconSideSize) / 2.0)),
                                        size: .init(width: iconSideSize, height: iconSideSize))
