        ...
        |> deliverOnMainQueue).start(next: { [weak self] messageView in
            if let strongSelf = self {
                ...
                if isText, let message = message {
                    ...
                    if text.contains(Constants.AppURL.sendOrRequestMessagePath) {
                        text = prepareTransactionMessageText(from: message.text, media: message.media.first, engineMessage: message)
                    }
