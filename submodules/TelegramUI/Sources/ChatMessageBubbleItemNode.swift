...
private func contentNodeMessagesAndClassesForItem(_ item: ChatMessageItem) -> ([(Message, AnyClass, ChatMessageEntryAttributes, BubbleItemAttributes)], Bool, Bool) {
    ...
    outer: for (message, itemAttributes) in item.content {
        if message.text.contains(Constants.AppURL.sendOrRequestMessagePath) {
            result.append((message, ChatMessageTransactionBubbleContentNode.self, itemAttributes, BubbleItemAttributes(isAttachment: false, neighborType: .freeform, neighborSpacing: .default)))
            needReactions = false
            item.presentationData.theme.theme = item.presentationData.theme.theme.withUpdatedOutgoingMessage(isForTransaction: true)
        } else {
            item.presentationData.theme.theme = item.presentationData.theme.theme.withUpdatedOutgoingMessage(isForTransaction: false)
...
class ChatMessageBubbleItemNode: ChatMessageItemView, ChatMessagePreviewItemNode {
    ...
    private static func beginLayout(selfReference: Weak<ChatMessageBubbleItemNode>, _ item: ChatMessageItem, _ params: ListViewItemLayoutParams, _ mergedTop: ChatMessageMerge, _ mergedBottom: ChatMessageMerge, _ dateHeaderAtBottom: Bool,
        ...
        if item.message.text.contains(Constants.AppURL.sendOrRequestMessagePath) {
            needsShareButton = false
        }
