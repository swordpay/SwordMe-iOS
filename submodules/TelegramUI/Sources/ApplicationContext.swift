...
final class AuthorizedApplicationContext {
        ...
        self.showCallsTabDisposable = (showCallsTabSignal |> deliverOnMainQueue).start(next: { [weak self] value in
            if let strongSelf = self {
                if strongSelf.showCallsTab != value {
                    ...
                    strongSelf.rootController.updateRootControllers(showCallsTab: false)
                }
            }
        })
    ...
    func openChatFromRedirection(peerId: PeerId, extraPeerId: PeerId) {
        let _ = combineLatest(
            queue: Queue.mainQueue(),
            self.context.engine.data.get(TelegramEngine.EngineData.Item.Peer.Peer(id: peerId)),
            ...
            let chatLocation: NavigateToChatControllerParams.Location
            chatLocation = .peer(correctPeer)
            
            self.context.sharedContext.navigateToChatController(NavigateToChatControllerParams(navigationController: self.rootController,
    ...
    func openUrl(_ url: URL) {
        // MARK: - SwordChanges // deeplink changes
        
        if let rootTabBar = self.rootController.rootTabController {
            if url.host == Constants.AppURL.deeplinkingBasePath && DeeplinkSegmentsProvider.isKnownPath(url.path),
               let selectedTabIndex = DeeplinkSegmentsProvider.tabIndex(for: url.path) {
                let handler: () -> Void = {
                    rootTabBar.selectedIndex = selectedTabIndex

                    let tabRootController = rootTabBar.controllers[selectedTabIndex]
                    guard let tabRootControllerNavController = tabRootController.navigationController as? TelegramRootController else { return }
                    
                    tabRootControllerNavController.popToRoot(animated: false)
                    let components = URLComponents(url: url, resolvingAgainstBaseURL: true)
                    if let stringedId = components?.queryItems?.first(where: { $0.name == "peer_id" })?.value as? String,
                        let stringedExtraPeerId = components?.queryItems?.first(where: { $0.name == "extra_peer_id" })?.value as? String,
                       let id = Int64(stringedId),
                       let extraId = Int64(stringedExtraPeerId) {

                        let peerId = PeerId(id)
                        let extraPeerId = PeerId(extraId)

                        self.openChatFromRedirection(peerId: peerId, extraPeerId: extraPeerId)
                        ...
                        let segments = DeeplinkSegmentsProvider.segments(from: url.path, queryItems: components?.queryItems ?? [])
                        guard !segments.isEmpty else { return }

                        DeeplinkManager.segments = segments
                        DeeplinkManager.navigate(to: tabRootController)
                    }

                }
                if let safariController = UIApplication.shared.topMostViewController() as? SFSafariViewController {
                ...
                self.context.sharedContext.openExternalUrl(context: self.context, urlContext: .generic, url: url.absoluteString.replacingOccurrences(of: "sm", with: "tg"), forceExternal: false, presentationData: presentationData, navigationController: self.rootController, dismissInput: { [weak self] in
