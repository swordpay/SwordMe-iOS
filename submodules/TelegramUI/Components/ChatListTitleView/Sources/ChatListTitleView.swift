...
private func titleFont(for text: String) -> UIFont {
    return Font.with(size: 17, design: .regular, weight: .semibold, traits: [.monospacedNumbers])
}
...
public final class ChatListTitleView: UIView, NavigationBarTitleView, NavigationBarTitleTransitionNode {
    ...
    public func setTitle(_ title: NetworkStatusTitle, animated: Bool) {
        ...
        if self._title != oldValue {
            var textColor: UIColor {
                return self.theme.rootController.navigationBar.primaryTextColor
            }

            self.titleNode.attributedText = NSAttributedString(string: self.title.text, font: titleFont(for: self.title.text), textColor: textColor)
    ...
    public func updateLayout(size: CGSize, clearBounds: CGRect, transition: ContainedViewLayoutTransition) {
        ...
        var titleFrame = titleContentRect
        var titleTransition = transition
        titleFrame.origin.x += 6
        let appIconFrame = CGRect(origin: .init(x: titleFrame.origin.x - 31, y: titleFrame.origin.y + 2),
                                  size: .init(width: 147, height: 35))
        ...
        let buttonX = max(0.0, appIconFrame.minX - 12.0 - 12.0)
        ...
        let lockFrame = CGRect(x: appIconFrame.minX - 12.0 - 12.0, y: appIconFrame.minY + 10.0, width: 2.0, height: 2.0)
        ...
        if let peerStatus = self.title.peerStatus {
            ...
            titleCredibilityIconTransition.setFrame(view: titleCredibilityIconView, frame: CGRect(origin: CGPoint(x: appIconFrame.maxX + 2.0, y: floorToScreenPixels(titleFrame.midY - titleIconSize.height / 2.0)), size: titleIconSize))
