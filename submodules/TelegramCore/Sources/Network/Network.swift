...
class Keychain: NSObject, MTKeychain {
    ...
    func setObject(_ object: Any!, forKey aKey: String!, group: String!) {
        ...
        MTContext.perform(objCTry: {
            ...
            do {
                ...
                print("NSKeyedArchiver.archivedData error \(error)")
            }
        })
    }
    ...
    func object(forKey aKey: String!, group: String!) -> Any! {
        ...
        if let data = self.get(group + ":" + aKey) {
            ...
            MTContext.perform(objCTry: {
                do {
                    let unarchiveResult = try NSKeyedUnarchiver.unarchivedObject(ofClasses: [NSArray.self,
                                                                                             NSDictionary.self,
                                                                                             NSNumber.self,
                                                                                             NSMutableData.self,
                                                                                             NSString.self,
                                                                                             MTTransportScheme.self,
                                                                                             MTTransportSchemeStats.self,
                                                                                             MTTransportSchemeKey.self,
                                                                                             MTDatacenterAuthInfo.self,
                                                                                             MTDatacenterAddress.self,
                                                                                             MTDatacenterAddressSet.self,
                                                                                             MTDatacenterSaltInfo.self],
                                                                                 from: data)
                    result = unarchiveResult
                } catch {
                    print("NSKeyedUnarchiver.unarchivedObject error \(error)")
                }
            })
            return result
        }
