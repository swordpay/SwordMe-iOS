        ...
        |> deliverOnMainQueue).start(next: { peer in
            if let user = peer as? TelegramUser, user.botInfo != nil {
                ...
                if !currentAddressName.isEmpty {
                    dismissInputImpl?()
                    let shareController = ShareController(context: context, subject: .url("https://\(Constants.swordMeDomain)/\(currentAddressName)"))
        ...
        if let peer = peer as? TelegramUser {
            ...
            rightNavigationButton = ItemListNavigationButton(content: .text(presentationData.strings.Common_Done), style: state.updatingAddressName ? .activity : .bold, enabled: doneEnabled, action: {
                ...
                if let updatedAddressNameValue = updatedAddressNameValue {
                    AppData.telegramUserUsername = updatedAddressNameValue.trimmingCharacters(in: .whitespacesAndNewlines).isEmpty ? (AppData.userPhoneNumber  ?? "") : updatedAddressNameValue
