...
private final class PremiumGiftScreenComponent: CombinedComponent {
    ...
    final class State: ComponentState {
        ...
        private func openPremimumAccountBotChat() {
            let link = "https://t.me/PremiumBot"
            guard let url = URL(string: link) else { return }
            
            let userInfo: [String: Any] = ["url": url]
            NotificationCenter.default.post(name: NSNotification.Name(rawValue: "usernameDidSelected"), object: self, userInfo: userInfo)
        }

        func buy() {
            openPremimumAccountBotChat()
