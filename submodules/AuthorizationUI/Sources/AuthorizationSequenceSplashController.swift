...
// MARK: - SwordChanges splash screen
public final class AuthorizationSequenceSplashController: ViewController {
    ...
    private lazy var skipButton: UIButton = {
       let button = UIButton()
        ...
        button.setTitleColor(UIColor(rgb: 0x3497F9), for: .normal)
        
        button.addTarget(self, action: #selector(skipButtonTouchUp), for: .touchUpInside)
    ...
    init(accountManager: AccountManager<TelegramAccountManagerTypes>, account: UnauthorizedAccount, theme: PresentationTheme) {
        ...
        self.controller = RMIntroViewController(backgroundColor: theme.list.plainBackgroundColor, primaryColor: theme.list.itemPrimaryTextColor, buttonColor: theme.intro.startButtonColor, accentColor: theme.list.itemAccentColor, regularDotColor: theme.intro.dotColor, highlightedDotColor: theme.intro.highlightedDotColor, suggestedLocalizationSignal: localizationSignal)
        ...
        self.controller.createStartButton = { [weak self] _ in
    ...
    var buttonFrame: CGRect {
        let startButtonWidth = min(430.0 - 48.0, self.view.bounds.size.width - 48.0);
        return .init(x: floor((self.view.bounds.size.width - startButtonWidth) / 2.0),
                     y: 2500,
    ...
    var buttonTitle: String {
        return self.skipButton.titleLabel?.text ?? ""
    }
    ...
    private func activateLocalization(_ code: String) {
        ...
        |> deliverOnMainQueue).start(next: { [weak self] currentCode, suggestedCode in
            ...
            strongSelf.startButton.alpha = 0.0
    ...
    @objc
    private func skipButtonTouchUp() {
        startButton.alpha = 0
