...
private class ApplicationStatusBarHost: StatusBarHost {
    ...
    var statusBarFrame: CGRect {
        return self.application.windows.first?.windowScene?.statusBarManager?.statusBarFrame ?? CGRect(origin: CGPoint(), size: CGSize(width: 320.0, height: 20.0))
    }
    
    var statusBarStyle: UIStatusBarStyle {
        get {
            return self.application.windows.first?.windowScene?.statusBarManager?.statusBarStyle ?? .default
        } set(value) {
...
//        FirebaseApp.configure()
        AppEnvironmentSetuper.global.setupEnvironment()
        ...
        |> deliverOnMainQueue).start(next: { context in
            ...
            if let context = context {
                ...
                |> deliverOnMainQueue).start(next: { [ weak self ] _ in
                    ...
                    AuthorizationManager.global.getUserData()
        ...
        |> deliverOnMainQueue).start(next: { context in
            ...
            // MARK: - SwordChange - initial controller
        ...
        if (UIApplication.shared.windows.first?.windowScene?.statusBarManager?.isStatusBarHidden ?? true) {
        ...
        bindToAppStateUpdate()
        bindToOpenURLNotification()
        bindToAuthorizationDidCompleted()
    ...
    private func bindToOpenURLNotification() {
        NotificationCenter.default.publisher(for: .usernameDidSelected)
    ...
    private func bindToAuthorizationDidCompleted() {
        NotificationCenter.default.publisher(for: .authorizationDidCompleted)
            .sink { [ weak self ] notification in
                guard let self else { return }
                
                self.mainWindow.viewController = self.contextValue?.rootController
            }
            .store(in: &cancellables)
    }
    
    private func bindToAppStateUpdate() {
