...
private let swordMeDomain = "t.me"
private final class BundleHelper: NSObject {
}

private var swordBundle: Bundle {
...
private func shareQrCode(context: AccountContext, link: String, ecl: String, view: UIView) {
    let _ = (qrCode(string: link, color: .black, backgroundColor: .white, icon: .custom(UIImage(named: "qr-app-icon", in: swordBundle, with: nil)), ecl: ecl)
...
public final class QrCodeScreen: ViewController {
    public enum Subject {
        ...
        var link: String {
            switch self {
                case let .peer(peer):
                    return "https://\(swordMeDomain)/\(peer.addressName ?? "")"
    ...
    class Node: ViewControllerTracingNode, UIScrollViewDelegate {
        ...
        init(context: AccountContext, presentationData: PresentationData, subject: QrCodeScreen.Subject) {
            ...
            self.qrImageNode.setSignal(qrCode(string: subject.link, color: .black, backgroundColor: .white, icon: .custom(UIImage(named: "qr-app-icon", in: swordBundle, with: nil)), ecl: subject.ecl) |> beforeNext { [weak self] size, _ in
