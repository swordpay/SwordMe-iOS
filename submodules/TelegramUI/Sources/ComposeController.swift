...
public class ComposeControllerImpl: ViewController, ComposeController {
    ...
    override public func loadDisplayNode() {
        ...
        self.contactsNode.requestOpenPeerFromSearch = { [weak self] peerId in
            self?.handlePeerIdSelection(peerId)
        }
        ...
        self.contactsNode.contactListNode.openPeer = { [weak self] peer, _ in
            if case let .peer(peer, _, _) = peer {
                self?.handlePeerIdSelection(peer.id)
            }
        }
...
extension ComposeControllerImpl {
    func handlePeerIdSelection(_ peerId: PeerId) {
        openSendOrReuest(for: peerId)
    }

    func openSendOrReuest(for peerId: PeerId) {
        let info: PayOrRequestStateInfoModel = .init(peerId: peerId,
                                                     source: ChannelPaymentViewModel.Source.createPayment,
                                                     isRequesting: true,
                                                     isSendActionAvailable: true)
        let controller = ViewControllerProvider.Channels.channelPayment(stateInfo: info, context: self.context)
