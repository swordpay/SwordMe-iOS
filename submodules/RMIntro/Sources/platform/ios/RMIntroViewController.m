...
- (instancetype)initWithBackgroundColor:(UIColor *)backgroundColor primaryColor:(UIColor *)primaryColor buttonColor:(UIColor *)buttonColor accentColor:(UIColor *)accentColor regularDotColor:(UIColor *)regularDotColor highlightedDotColor:(UIColor *)highlightedDotColor suggestedLocalizationSignal:(SSignal *)suggestedLocalizationSignal
{
    ...
    if (self != nil)
    {
        ...
        NSArray<NSString *> *descriptions = @[
            @"Pay, Send, Exchange and Withdraw.",
            @"Send Money, Crypto. Anytime. Anywhere.",
        ];
        
        NSArray<NSString *> *imagesNames = @[
            @"onboarding-send-or-request-icon",
            @"onboarding-anytime-icon",
        ];
...
//        }];
        ...
        NSBundle *mainBundle = [NSBundle bundleForClass:[BundleHelper class]];
        NSString *path = [mainBundle pathForResource:@"SwordAppArchBundle" ofType:@"bundle"];
        NSBundle *swordBundle = [[NSBundle alloc] initWithPath:path];

        self->_mainBundle = swordBundle;
    }
    return self;
}
...
BOOL isImageSetup = NO;

- (void)setupSlidImageView {
    if (!isImageSetup) {
        bool isIpad = ([UIDevice currentDevice].userInterfaceIdiom == UIUserInterfaceIdiomPad);
        
        CGFloat size = [self imageViewSize];
        ...
        _imageView = [[UIImageView alloc] initWithFrame: CGRectMake(self.view.bounds.size.width / 2 - size / 2, height, size, size)];
        _imageView.image = [UIImage imageNamed:_imagesNames[0] inBundle: _mainBundle withConfiguration: nil];
...
- (void)updateLayout
{
    ...
    CGFloat pageY = 0;
    CGFloat imageSize = [self imageViewSize];
    ...
    _imageView.frame = CGRectChangedOriginY(_imageView.frame, glViewY - statusBarHeight);
    ...
    startButton.frame = CGRectMake((self.view.bounds.size.width - 100), 50, 80, 50.0f);
    ...
    [_pageViews enumerateObjectsUsingBlock:^(UIView *pageView, NSUInteger index, __unused BOOL *stop) {
        pageView.frame = CGRectMake(index * self.view.bounds.size.width, (pageY - statusBarHeight), self.view.bounds.size.width, 320);
    }];
}
...
// MARK: - Sword change should be skip button handler
...
- (void)scrollViewDidScroll:(UIScrollView *)scrollView
{
    ...
    if (justEndDragging)
    {
        ...
        _currentPage = MAX(0, MIN(1, _currentPage));
    ...
    _imageView.image = [UIImage imageNamed:_imagesNames[_currentPage] inBundle: _mainBundle withConfiguration: nil];
}
