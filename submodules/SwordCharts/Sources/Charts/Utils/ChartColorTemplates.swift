...
open class ChartColorTemplates: NSObject
{
    ...
    @objc open class func colorFromString(_ colorString: String) -> NSUIColor
    {
        ...
        else if colorString.hasPrefix("rgba")
        {
            ...
            let _ = scanner.scanString("rgba")
            let _ = scanner.scanCharacters(from: leftParenCharset)
            scanner.scanHexInt64(&r)
            let _ = scanner.scanCharacters(from: commaCharset)
            scanner.scanHexInt64(&g)
            let _ = scanner.scanCharacters(from: commaCharset)
            scanner.scanHexInt64(&b)
            let _ = scanner.scanCharacters(from: commaCharset)
            scanner.scanHexFloat(&a)
            return NSUIColor(
                red: CGFloat(r) / 255.0,
